...
1. Clean previous Mosquitto configuration
```bash
...
```
Common Name (CN):Broker IP must.

4. Move certificates and set permissions
```bash
...
5. Create MQTT username & password (Not mandatory but recomended)
```bash
sudo mosquitto_passwd -c /etc/mosquitto/passwd myuser
```

6️. Create secure Mosquitto configuration
Open the config editor:
```bash
sudo nano /etc/mosquitto/conf.d/secure.conf
```
--Paste this inside the editor (then save & exit):
```bash
listener 8883
protocol mqtt
# TLS configuration
...
tls_version tlsv1.2
ciphers ECDHE-RSA-AES128-GCM-SHA256 #Change it per your requirment
# Authentication
allow_anonymous false
password_file /etc/mosquitto/passwd
```

--Restart Mosquitto: (Must)
```bash
...
7️. Generate ca_cert.h for ESP32
```bash
...
mosquitto_sub -h localhost -p 8883 \
  ...
  -t "esp32/env_data" -v
```
You should now see JSON messages published by the ESP32 securely over TLS.
